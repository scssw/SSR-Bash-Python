...
if [[ "$sv" == "$solve" ]];then
	...
	if [[ -e ${bakfile} ]];then
		...
		if [[ -e ./timelimit.db ]];then
			mv -f ./timelimit.db /usr/local/SSR-Bash-Python/timelimit.db
		fi
...
[Service]
Type=simple
ExecStart=/usr/bin/env python3 /usr/local/shadowsocksr/server.py
Restart=always
RestartSec=5
...
EOF
		...
		(crontab -l 2>/dev/null; echo "@reboot cd /usr/local/shadowsocksr && python3 server.py&& python3 server.py python3 server.py > /dev/null 2>&1 &") | crontab -
		# 立即启动web面板
		cd /usr/local/shadowsocksr && nohup python3 server.py > /dev/null 2>&1 &
